// mdl_asio_helper.cpp - Byte packing, transmit via TCP (ASIO), unpacking.

// Source code for GMP_SIL_Core (TCP Version)
...
// default timeout 10s (10_000 ms)
#define GMP_ASIO_TCP_LINK_TIMEOUT (10000)

// Simulink S-Function acts as TCP Client (controller C++ program is Server).
// Do NOT define ASIO_TCP_HELPER_SERVER_MODE here.
...
static void mdlStart(SimStruct* S)
{
    ...
    asio_tcp_helper* tcp_helper = (asio_tcp_helper*)ssGetPWorkValue(S, 0);
    if (tcp_helper != nullptr)
    {
        delete tcp_helper;
        sprintf(msg, "%s: TCP Helper object hasn't been released.", DRIVER);
        ssWarning(S, msg);
    }

    // tcp_helper = new asio_tcp_helper(target_ip, recv_port, trans_port, cmd_recv_port, cmd_trans_port);
    tcp_helper = new asio_tcp_helper(ipaddr.str(), recv_port, trans_port, cmd_recv_port, cmd_trans_port);

    if (tcp_helper == nullptr)
    {
        sprintf(msg, "%s: ASIO TCP helper cannot create an instance!", DRIVER);
        delete tcp_helper;
        tcp_helper = nullptr;

        ssSetPWorkValue(S, 0, tcp_helper);
    ...
    if (!network_json_file.is_open())
    {
        ...
        sprintf(msg, "%s: network.json is generated, this script is for controller program!\r\n", DRIVER);
        ssPrintf(msg);
    }
    else
    {
    ...
    try
    {
        sprintf(msg, "%s: Connecting to target (TCP): %s.\r\n", DRIVER, ipaddr.str().c_str());
        ssPrintf(msg);

        // Create TCP connection (Client mode: connect with retry)
        tcp_helper->connect_to_target();

        // Set Overtime
        tcp_helper->set_overtime();
    }
    //catch (boost::system::system_error &e)
    catch (const std::exception& e)
    {
        tcp_helper->release_connect();
        ...
        sprintf(msg, "%s: ASIO TCP helper, Cannot connect to Target! Source exception: %s", DRIVER, e.what());
        ssSetErrorStatus(S, msg);
        return;
    }
    ...
    tcp_helper->send_cmd(cmd.c_str(), (uint32_t)cmd.length());
}
...
static void mdlOutputs(SimStruct* S, int_T tid)
{
    ...
    // Get tcp handle
    asio_tcp_helper* tcp_helper;
    tcp_helper = (asio_tcp_helper*)ssGetPWorkValue(S, 0);
    ...
    try
    {
        tcp_helper->send_msg((char*)tranBuffer, desired_packed_width);
    }
    //catch (boost::system::system_error &e)
    catch (const std::exception& e)
    {
        ...
        sprintf(msg, "%s: ASIO TCP helper, Cannot transmit message! Source exception: %s", DRIVER, e.what());
        ssSetErrorStatus(S, msg);
        return;
    }
    ...
    try
    {
        ...
        if (tcp_helper->recv_msg((char*)recvBuffer, desired_unpacked_width))
        {
            sprintf(msg, "%s: ASIO TCP helper, Receive failed or connection lost, please start SIL Controller first!", DRIVER);
            ssSetErrorStatus(S, msg);
        }
    }
    //catch (boost::system::system_error &e)
    catch (const std::exception& e)
    {
        ...
        sprintf(msg, "%s: ASIO TCP helper, Cannot receive message! Source exception: %s", DRIVER, e.what());
        ssSetErrorStatus(S, msg);
        return;
    }
...
static void mdlTerminate(SimStruct* S)
{
    ...
    sprintf(msg, "%s: mdlTerminate is invoked\r\n", DRIVER);
    ssPrintf(msg);

    if (tcp_helper != nullptr)
    {
        // Send Stop command before releasing (must check non-null first!)
        try
        {
            std::string cmd("Stop");
            tcp_helper->send_cmd(cmd.c_str(), (uint32_t)cmd.length());
        ...
        {
            sprintf(msg, "%s: [ASIO-TCP] Failed to send Stop command: %s\r\n", DRIVER, e.what());
            ssPrintf(msg);
        }
